...
parser.add_argument(
    "--season", required=True, help="year from 2014 to 2024 plus NH or SH"
)
parser.add_argument(
...
parser.add_argument("--reg_weight_decay", default=0.005, type=float, help="Weight decayin AdamW for regressor")
...
outputs_path = f"{storage_path}/Season_based_split_performance/{VERSION}/trained_until_{SEASON_NAME}/"
...
seasons = {
    "2014SH": {"start_date": "2014-02-01", "end_date": "2014-08-31"},
    "2014NH": {"start_date": "2014-09-01", "end_date": "2015-01-31"},
    "2015SH": {"start_date": "2015-02-01", "end_date": "2015-08-31"},
    "2015NH": {"start_date": "2015-09-01", "end_date": "2016-01-31"},
    "2016SH": {"start_date": "2016-02-01", "end_date": "2016-08-31"},
    "2016NH": {"start_date": "2016-09-01", "end_date": "2017-01-31"},
    "2017SH": {"start_date": "2017-02-01", "end_date": "2017-08-31"},
    "2017NH": {"start_date": "2017-09-01", "end_date": "2018-01-31"},
    "2018SH": {"start_date": "2018-02-01", "end_date": "2018-08-31"},
    "2018NH": {"start_date": "2018-09-01", "end_date": "2019-01-31"},
    "2019SH": {"start_date": "2019-02-01", "end_date": "2019-08-31"},
    "2019NH": {"start_date": "2019-09-01", "end_date": "2020-01-31"},
    "2020SH": {"start_date": "2020-02-01", "end_date": "2020-08-31"},
    "2020NH": {"start_date": "2020-09-01", "end_date": "2021-01-31"},
    "2021SH": {"start_date": "2021-02-01", "end_date": "2021-08-31"},
    "2021NH": {"start_date": "2021-09-01", "end_date": "2022-01-31"},
    "2022SH": {"start_date": "2022-02-01", "end_date": "2022-08-31"},
    "2022NH": {"start_date": "2022-09-01", "end_date": "2023-01-31"},
    "2023SH": {"start_date": "2023-02-01", "end_date": "2023-08-31"},
    "2023NH": {"start_date": "2023-09-01", "end_date": "2024-01-31"},
    "2024SH": {"start_date": "2024-02-01", "end_date": "2024-08-31"},
    "2024NH": {"start_date": "2024-09-01", "end_date": "2025-01-31"},
}

cutoff_date = seasons[SEASON_NAME]["end_date"]
...
#read fasta sequence as data frame
...
DA_df = DA_df[DA_df['date_corrected'] <= cutoff_date]
...
print(f"Start performing antigenicity prediction using season {SEASON_NAME} data ...")

train_data_path = (
    dataset_stem_dir_path
    + f"data/dataset_SplitBySeasons_final_strict_score_250124/WHO_GISAID_dataset_final_strict_score_{SEASON_NAME}_train.csv"
)
test_data_path = (
    dataset_stem_dir_path
    + f"data/dataset_SplitBySeasons_final_strict_score_250124/WHO_GISAID_dataset_final_strict_score_{SEASON_NAME}_test.csv"
)

WHO_GISAID_dataset_train = pd.read_csv(train_data_path)
WHO_GISAID_dataset_test = pd.read_csv(test_data_path)
WHO_GISAID_dataset_train["dataset_type"] = "train"
WHO_GISAID_dataset_test["dataset_type"] = "test"
WHO_GISAID_dataset_df = pd.concat(
    [WHO_GISAID_dataset_train, WHO_GISAID_dataset_test], axis=0
)
...
selected_df_train_val = selected_df[selected_df["dataset_type"] == "train"]
selected_df_test = selected_df[selected_df["dataset_type"] == "test"]
...
# Example usage
# Assuming selected_df_train_val contains 'virus' and 'year' columns
selected_df_train, selected_df_val = split_dataframe_with_stratified_group_kfold(selected_df_train_val)
...
# virus unique dataset
selected_df_train_unique = selected_df_train.drop_duplicates(subset=['virus'])
selected_df_train_sampled = selected_df_train_unique.groupby('virus_collection_year', group_keys=False).apply(lambda x: x.sample(n=min(len(x), 20)))
...
encodes_virus_train_virus_only = tokenize_sequences(
    selected_df_train_sampled["virus_seq"].tolist(), tokenizer, MAX_LENGTH
)
...
# concatenate list to obtain final prediction results
...
# perform prediction batch by batch, test
with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.float16):
...
print("encodes_virus_all_HA", encodes_virus_all_HA)
print("dataset_virus_only", dataset_virus_only)
                ...
                if hasattr(dataset, "get_unique_combinations_indices"):
                    ...
                    for indices in unique_combinations.values():
                        if len(indices) <= self.num_samples_per_combination:
                            ...
                            # Use `torch.randperm()` instead of `random.sample()`
...
# iterate over the seasons and select the rows with 'virus_collection_date' is within start_date and end_date
seasons_df = {}
for season, dates in seasons.items():
    seasons_df[season] = selected_df_test[
        (selected_df_test["virus_collection_date"] >= dates["start_date"])
        & (selected_df_test["virus_collection_date"] <= dates["end_date"])
        & (selected_df_test["reference_collection_date"] <= dates["end_date"])
    ]

# Add "all season" by combining all seasonal data
seasons_df["all_season"] = selected_df_test[
    (selected_df_test["virus_collection_date"] >= "2014-02-01")
]

# %%
# summarize prediction score of censor-capped data # TODO: add genetic correlation
seasons_performance_score_censor_cap = {}
for season, df in seasons_df.items():
    if df.empty:
        seasons_performance_score_censor_cap[season] = (
            None,
            None,
            None,
            None,
        ...
        continue
    pearson = df["score"].corr(df["predicted_dist_censor_cap"])
    spearman = df["score"].corr(df["predicted_dist_censor_cap"], method="spearman")
    mae = mean_absolute_error(df["score"], df["predicted_dist_censor_cap"])

    pearson_cart = df["score"].corr(df["predicted_dist_cartography_censor_cap"])
    spearman_cart = df["score"].corr(
        df["predicted_dist_cartography_censor_cap"], method="spearman"
    )
    mae_cart = mean_absolute_error(
        df["score"], df["predicted_dist_cartography_censor_cap"]
    )
...
seasons_performance_score_censor_cap_df = pd.DataFrame(
    seasons_performance_score_censor_cap,
    index=[
        "Pearson",
        "Spearman",
...
path_to_save_performance_score_censor_cap = (
    outputs_path + f"performance_score_censor_cap_{SEASON_NAME}.csv"
)
seasons_performance_score_censor_cap_df.to_csv(
    path_to_save_performance_score_censor_cap
)
print(
    f"Performance of prediction score (censor-capped) for each season saved to {path_to_save_performance_score_censor_cap}"
)
...
path_to_save_test_df = (
    outputs_path + f"test_df_{SEASON_NAME}.csv"
)
